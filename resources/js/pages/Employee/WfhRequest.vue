  ...
  <div class="min-h-screen bg-gradient-to-br from-navy via-slate-800 to-blue-900">
    ...
    <main class="max-w-2xl mx-auto px-4 py-8">
      ...
      <form v-else @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <button type="submit" :disabled="loading" class="w-full py-4 bg-emerald-500 hover:bg-emerald-600 disabled:bg-emerald-500/50 rounded-xl text-white font-bold text-lg transition-colors flex items-center justify-center gap-2">
...
async function handleSubmit() {
  ...
  error.value = ''
  try {
    const res = await axios.post('/api/employee/wfh-requests', form)
    if (res.data.success) {
      success.value = true
    } else {
